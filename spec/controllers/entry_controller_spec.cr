...
def entry_hash
  {"device" => "Fake", "date_string" => Time.now, "sgv" => "1", "delta" => "1.00", "direction" => "Flat", "filtered" => "1", "unfiltered" => "1", "rssi" => "1", "noise" => "1", "sys_time" => Time.now}
end

def entry_params
  params = [] of String
  params << "device=#{entry_hash["device"]}"
  params << "date_string=#{entry_hash["date_string"].to_s}"
  ...
  params << "sys_time=#{entry_hash["sys_time"].to_s}"
  params.join("&")
end
